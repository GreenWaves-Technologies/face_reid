...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, act_relu=False):
        ...
        if self.stride > 1:
            self.branch1 = nn.Sequential(
                ...
                nn.ReLU6() if act_relu else nn.PReLU(),
            )
        else:
            self.branch1 = nn.Sequential()
...
class ShuffleFaceNet(nn.Module):
    ## ShuffleFaceNet x1.5
    # def __init__(self, stages_repeats=[4, 8, 4], stages_out_channels=[24, 176, 352, 704, 1024], inverted_residual=InvertedResidual,no_resize=True, act_relu=False):
    ## ShuffleFaceNet x1.0
    #def __init__(self, stages_repeats=[4, 8, 4], stages_out_channels=[24, 116, 232, 464, 1024], inverted_residual=InvertedResidual,no_resize=True, act_relu=False):
    ## ShuffleFaceNet x0.5
    def __init__(self, stages_repeats=[4, 8, 4], stages_out_channels=[24, 48, 96, 192, 1024], inverted_residual=InvertedResidual, no_resize=True, act_relu=True):
        ...
        for name, repeats, output_channels in zip(
            ...
            seq = [inverted_residual(input_channels, output_channels, 2, act_relu)]
            for i in range(repeats - 1):
                seq.append(inverted_residual(output_channels, output_channels, 1, act_relu))
...
if __name__ == "__main__":
    ...
    net = ShuffleFaceNet(no_resize=True, act_relu=True)
    print(net)
    print(sum(p.numel() for p in net.parameters()))
    macs, params = get_model_complexity_info(
        net, (3, 112, 112), as_strings=True,
    ...
    torch.onnx.export(net, dummy_input, "shufflenet_relu.onnx")
    # x = net(input)
    # print(x.shape)
